...
async def get_gbif_species_by_county_async(
    ...
    - Sequential (old): N counties × 1 query each = N sequential calls
...
def get_gbif_species_by_county(
    lat: float,
    lon: float,
    buffer_miles: float = 25.0,
    threatened_only: bool = True,
...
def format_occurrences_summary(gbif_data: Dict, min_year: int, threatened_only: bool, csv_file: str) -> str:
    """
    ...
    lines.append(f"CSV Export: {csv_file}")
    lines.append("")
...
def format_species_by_county_summary(county_data: Dict, min_year: int, threatened_only: bool, csv_file: str) -> str:
    """
