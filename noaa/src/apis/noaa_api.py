...
def get_noaa_critical_habitat_in_roi(lat: float, lon: float, buffer_miles: float = 25.0) -> Dict:
    ...
    labels and summing area/length across fragments.
...
def _query_noaa_ch_layers(buffer_geometry: Dict) -> tuple[List[Dict], List[str]]:
    ...
    for layer_id, layer_name in NOAA_WCR_CH_LAYERS.items():
        ...
        try:
            ...
            deduped = _deduplicate_fragments(result.features, layer_id, layer_name)
...
def _deduplicate_fragments(features: List[Dict], layer_id: int, layer_name: str) -> List[Dict]:
    """
    ...
    for key, habitat in grouped.items():
        ...
        habitat["area_sqkm"] = round(habitat["area_sqkm"], 2) if habitat["area_sqkm"] else None
...
def format_noaa_critical_habitat_summary(result: Dict) -> str:
    ...
    if not habitats:
        ...
        for entity, records in sorted(by_species.items()):
            ...
            size_parts = []
            if total_area:
                size_parts.append(f"{round(total_area, 2)} sq km")
            if total_length:
                size_parts.append(f"{round(total_length, 2)} km")
            if size_parts:
                lines.append(f"- Combined intersecting extent: {', '.join(size_parts)}")
            lines.append("")
